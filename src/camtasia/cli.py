...
def _check_cli_deps():
    """Check that CLI dependencies are installed, exit with helpful message if not."""
    try:
        import docopt_subcommands  # noqa: F401
        import exit_codes  # noqa: F401
    except ImportError:
        print(
            "The pycamtasia CLI requires extra dependencies.\n"
            "Install them with: pip install pycamtasia[cli]",
            file=sys.stderr,
        )
        sys.exit(1)
...
def _register_commands():
    """Register all CLI subcommands. Must be called after _check_cli_deps."""
...
def main(argv=None):
    ...
    import docopt_subcommands as dsc  # type: ignore[import-not-found]
    from exit_codes import ExitCodeError  # type: ignore[import-not-found]
