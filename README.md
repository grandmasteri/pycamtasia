...
- Context manager for automatic save-on-exit
...
Use the context manager to auto-save:

```python
with camtasia.use_project('path/to/project.cmproj') as proj:
    # edits are saved automatically on exit
...
- **Project management**: `load_project()`, `use_project()`, `new_project()`, `Project`
