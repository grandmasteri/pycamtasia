...
class TestBasicOperations:
    ...
    def test_add_image_clip_opens(self, tmp_path, project):
    ...
    def test_add_and_remove_clip_opens(self, project):
    ...
    def test_add_transition_opens(self, project):
    ...
    def test_add_effect_opens(self, tmp_path, project):
...
class TestEffectsAndAnnotations:
    @pytest.mark.integration
    def test_color_adjustment_opens(self, tmp_path, project):
        img = _create_test_image(tmp_path)
        media = project.import_media(img)
        track = project.timeline.add_track('Content')
        clip = track.add_clip('IMFile', media.id, 0, seconds_to_ticks(5))
    ...
    @pytest.mark.integration
    def test_border_effect_opens(self, tmp_path, project):
        img = _create_test_image(tmp_path)
        media = project.import_media(img)
        track = project.timeline.add_track('Content')
        clip = track.add_clip('IMFile', media.id, 0, seconds_to_ticks(5))
    ...
    @pytest.mark.integration
    def test_callout_with_behavior_opens(self, project):
    ...
    @pytest.mark.integration
    def test_lower_third_opens(self, project):
        track = project.timeline.add_track('Titles')
        track.add_lower_third('Speaker Name', 'Title goes here', start_seconds=0.0, duration_seconds=5.0)
        project.save()
        assert _validate_in_camtasia(str(project.file_path)) == 0

    @pytest.mark.integration
    def test_multiple_tracks_opens(self, tmp_path, project):
        img = _create_test_image(tmp_path)
        media = project.import_media(img)
        for i in range(5):
            track = project.timeline.add_track(f'Track {i}')
            track.add_clip('IMFile', media.id, 0, seconds_to_ticks(5))
        project.save()
        assert _validate_in_camtasia(str(project.file_path)) == 0
...
class TestNewEffects:
    @pytest.mark.integration
    def test_colorize_effect_opens(self, tmp_path, project):
        img = _create_test_image(tmp_path)
        media = project.import_media(img)
        track = project.timeline.add_track('Content')
        clip = track.add_clip('IMFile', media.id, 0, seconds_to_ticks(5))
    ...
    @pytest.mark.integration
    def test_spotlight_effect_opens(self, tmp_path, project):
        img = _create_test_image(tmp_path)
        media = project.import_media(img)
        track = project.timeline.add_track('Content')
        clip = track.add_clip('IMFile', media.id, 0, seconds_to_ticks(5))
    ...
    @pytest.mark.integration
    def test_multiple_effects_opens(self, tmp_path, project):
        img = _create_test_image(tmp_path)
        media = project.import_media(img)
        track = project.timeline.add_track('Content')
        clip = track.add_clip('IMFile', media.id, 0, seconds_to_ticks(5))
    ...
    @pytest.mark.integration
    def test_dissolve_transition_opens(self, project):
        media = project.import_media(EMPTY_WAV)
        track = project.timeline.add_track('Audio')
        c1 = track.add_audio(media.id, start_seconds=0.0, duration_seconds=2.0)
        c2 = track.add_audio(media.id, start_seconds=2.0, duration_seconds=2.0)
...
class TestAdvancedOperations:
    ...
    @pytest.mark.integration
    def test_media_matte_opens(self, tmp_path, project):
    ...
    @pytest.mark.integration
    def test_title_card_opens(self, project):
...
class TestBehaviorPresets:
    @pytest.mark.integration
    def test_fade_behavior_opens(self, project):
        track = project.timeline.add_track('Callouts')
        callout = track.add_callout('Hello World', start_seconds=0.0, duration_seconds=5.0)
    ...
    @pytest.mark.integration
    def test_fly_in_behavior_opens(self, project):
        track = project.timeline.add_track('Callouts')
        callout = track.add_callout('Hello World', start_seconds=0.0, duration_seconds=5.0)
